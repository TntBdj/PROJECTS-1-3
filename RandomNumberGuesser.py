...
#user guess game
    if Game == "ug":
        x = randint(1, 100)
        SecretNumber = randint(x, 100)
        Guess = int(input(f"Try and guess the secret number. It's from {x} to 100: "))
...
#computer guess game (my version of the game)
        ...
        x = randint(1, 100)
        SecretNumber = int(input(f"Make a secret number? ({x}-100): "))
        y = 100
        Guess = randint(x, y)
        NumberOfGuesses = 1
        while Guess != SecretNumber:
...
#user computer guess game (the way they did it on the tutorial)
        ...
        y = 100
        x = randint(1, 100)
        print(f"think of a secret number? ({x}-100): ")
        Guess = randint(x, y)
        while x != y:
            print(Guess)
